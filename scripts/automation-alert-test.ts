...
(globalThis as any).fetch = async (input: unknown, init: any = {}) => {
  ...
  if (url.includes('/rest/v1/servers?') && url.includes('id=eq.srv-pve')) return new Response('[]', { status: 200 });
  if (url.includes('/rest/v1/notification_channels?') && url.includes('id=eq.chan-1') && method === 'PATCH') { channelPatches.push(JSON.parse(init.body)); return new Response('', { status: 204 }); }
...
const req = { method: 'POST', headers: { cookie: 'syncera_session=test-session' }, body: { type: 'proxmox_health_check', serverId: 'srv-pve' } };
const res = {
  code: 0,
