const baseUrl = (process.env.SUPABASE_URL || 'https://Supa.kidut.online').replace(/\/$/, '').replace(/\/rest\/v1$/, '');
...
export default async function handler(_req: any, res: any) {
  try {
    ...
    res.status(500).json({ error: err instanceof Error ? err.message : 'dashboard fetch failed' });
  }
}
